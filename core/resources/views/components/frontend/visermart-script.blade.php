<script>
    ...
    function buyNowClickHandler() {
        ...
        let attributeValues = button.parents('.product-details').find('.attributeBtn.active').map((i, element) => $(element).data('attribute').id).toArray();
        ...
        let quantity = button.parent().find('[name=quantity]').val();
        if (quantity == null) {
            quantity = button.siblings('.product-add-to-cart').find('[name=quantity]').val() || button.parents('.product-details').find('[name=quantity]').val() || 1;
        }
    ...
    $(document).on('click', '.buyNow', buyNowClickHandler);
