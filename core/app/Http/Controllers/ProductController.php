...
class ProductController extends Controller {
    ...
    private function getProducts($scope = null, $breadcrumbLinks = []) {
        ...
        if ($request->rating) {
            $allProducts->whereHas('reviews', function ($query) use ($request) {
                $query->where('status', Status::REVIEW_APPROVED)
    ...
    private function productCategories($productsQuery) {
        return $productsQuery->selectRaw('category_product.category_id, categories.name, categories.slug')
    ...
    public function reviews(Request $request, $id) {
        $reviews   = ProductReview::approved()->with(['user', 'productReviewImage', 'productReviewReply.admin:id,image,name', 'productReviewReply.productReviewReplyImage:id,product_review_reply_id,image'])->where('product_id', $id)->latest()->paginate(getPaginate(5));
