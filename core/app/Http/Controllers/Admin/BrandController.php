...
class BrandController extends Controller {
    ...
    public function downloadBrands(Request $request) {
        ...
        $brands = Brand::all();
        $filename = 'brands_' . time();
        $columns = ['id', 'name'];
        $headings = ['ID', 'Name'];

        $downloader = new Downloader($filename, $brands, $columns, $headings);

        if ($request->type == 'pdf') {
            return $downloader->downloadAsPdf('All Brands');
        } else {
            return $downloader->downloadAsCsv($brands);
        }
