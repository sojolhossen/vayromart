...
class CategoryController extends Controller {
    ...
    public function downloadCategories(Request $request) {
        ...
        $filename = 'categories_' . time();
        $columns = ['id', 'name', 'parent_id'];
        $headings = ['ID', 'Name', 'Parent ID'];
        $downloader = new Downloader($filename, $categories, $columns, $headings);

        if ($request->type == 'pdf') {
            return $downloader->downloadAsPdf('All Categories');
        } else {
            return $downloader->downloadAsCsv($categories);
        }
    }
}
