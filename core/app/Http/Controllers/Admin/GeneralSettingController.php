...
class GeneralSettingController extends Controller {
    ...
    public function systemConfigurationSubmit(Request $request) {
        ...
        $general->cod = $request->cod ? Status::ENABLE : Status::DISABLE;
        $general->product_wishlist = $request->product_wishlist ? Status::ENABLE : Status::DISABLE;
        ...
        $general->product_review_auto_approval = $request->product_review_auto_approval ? Status::ENABLE : Status::DISABLE;
        $general->guest_checkout = $request->guest_checkout ? Status::ENABLE : Status::DISABLE;
        $general->save();
    ...
    public function updateHomepageLayout(Request $request) {
        $request->validate([
            'name' => 'required|in:' . implode(',', homepageLayouts(true))
        ]);

        $general = gs();
        $general->homepage_layout =  $request->name;
        $general->save();

        $notify[] = ['success', 'Homepage layout switched successfully'];
        return back()->withNotify($notify);
    }
}
