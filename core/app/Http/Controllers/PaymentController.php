...
class PaymentController extends Controller {
    ...
    public function completeCheckout(Request $request) {
        ...
        return redirect()->route("checkout.confirmation", ['order' => $order->order_number])->withNotify($notify);
    }
    ...
    private function saveOrder($subtotal, $coupon, $gatewayCurrency, $cartData, $hasPhysicalProduct) {
        ...
        $order->guest_id     = $guestUser ? $guestUser->id : null;
        ...
        $order->shipping_method_id = $shippingMethod ? $shippingMethod->id : 0;
        $order->shipping_charge    = $shippingMethod ? $shippingMethod->charge : 0;
        $order->is_cod             = $gatewayCurrency->id ? 0 : 1;
        $order->payment_status     = Status::PAYMENT_INITIATE;
        $order->subtotal           = $subtotal;
        $order->total_amount       = getAmount($subtotal + ($shippingMethod ? $shippingMethod->charge : 0) - $couponAmount);
        $order->save();
