...
class NotifyProcess{
    ...
    */
	protected function getMessage(){
		...
		if ($user && $template) {
		    $message = $this->replaceShortCode(@$user->fullname ?? $user->name,
            @$user->username ??  str_replace(' ', '_', strtolower($user->name)),

            gs($globalTemplate),$template->$body);
    ...
    */
	public function createLog($type){
		...
		if ($this->user && $this->createLog) {
            ...
            if (@$this->user->id && \Illuminate\Support\Facades\Schema::hasColumn($notificationLog->getTable(), $userColumn)) {
